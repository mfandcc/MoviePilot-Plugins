...
def __handle_transfer_intercept_history_clear(self, mediainfo: MediaInfo, target_path: Path):
        ...
        with lock:
            ...
            # --- 新增的动态任务生成逻辑 ---
            if not task:
                logger.info(f"未在洗版任务缓存中找到记录，尝试根据有效洗版订阅动态生成任务...")
                
                # 查找匹配的有效洗版订阅
                # 注意： get_by 返回的是列表
                wash_subscribes = self.subscribe_oper.get_by(
                ...
                if wash_subscribes:
                    if mediainfo.type == MediaType.TV:
                        # 剧集需要精确匹配季
                        for sub in wash_subscribes:
                            # 确保订阅状态是活跃的 (N-新增, R-订阅中, P-待定)
                            if sub.season == mediainfo.season_num and sub.state in ["N", "R", "P"]:
                                wash_subscribe = sub
                                break
                    else:
                        # 电影直接取第一个活跃的
                        for sub in wash_subscribes:
                            if sub.state in ["N", "R", "P"]:
                                wash_subscribe = sub
                                break

                if not wash_subscribe:
                    logger.debug(f"未找到与 TMDB ID {mediainfo.tmdb_id} 匹配的有效洗版订阅，跳过洗版清理。")
                    return

                logger.info(f"找到匹配的洗版订阅: {self.__format_subscribe(wash_subscribe)}，开始生成清理任务。")
                ...
                if not histories:
                    logger.info(f"虽然找到洗版订阅，但未找到需要清理的旧整理记录，继续正常整理流程。")
                    # 即使没有旧文件，也应该继续，因为这本身就是一次合法的整理
                    # 动态构建一个没有历史记录的任务对象
                    task = {
                        "subscribe_id": wash_subscribe.id,
                        "subscribe_desc": self.__format_subscribe_desc(subscribe=wash_subscribe),
                        "subscribe_image": self.__get_subscribe_image(subscribe=wash_subscribe),
                        "histories": [],
                        "time": time.time(),
                    }
                else:
            ...
            if self.__clear_transfer_dest_histories(task=task, mediainfo=mediainfo, target_path=target_path):
                # 如果任务不是动态生成的（即来自缓存），则清理缓存
                if not is_dynamic_task and clear_key in tasks:
                    del tasks[clear_key]
                    self.__save_data(key="best_version_clear_histories", value=tasks)
